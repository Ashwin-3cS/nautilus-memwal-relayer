...
app.post("/walrus/upload", express.json({ limit: JSON_LIMIT_WALRUS_UPLOAD }), async (req, res) => {
    try {
        ...
        // Only needed for the Enoki sponsor path: Enoki rejects GasCoin as a
        // tx argument, so we swap the intent to use an explicit SUI coin.
        // Direct signing resolves gas normally — patching would break it.
        if (enokiApiKey) {
